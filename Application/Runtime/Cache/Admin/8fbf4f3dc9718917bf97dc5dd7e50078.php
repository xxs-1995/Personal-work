                ...
                <div id="Right">
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                                ...
                                <td><?php echo ($vo["bow_money"]); ?></td>
                                <td><?php echo ($vo["zec_money"]); ?></td>
                                <td><?php echo ($vo["pay_code"]); ?></td>
                                <td><?php if($vo["status"] == 0): ?>待审核<?php elseif($vo["status"] == 1): ?>已批准<?php else: ?>已拒绝<?php endif; ?></td>
                                ...
                                <td>
                                    <?php if($vo['status'] == 0): ?>[ <a link="<?php echo U('Personal/top_up_edit/',array('id'=>$vo['id'],'user_id'=>$vo['user_id'],'status'=>'1'));?>" href="javascript:void(0)"  class="ok">批准 </a> ][ <a link="<?php echo U('Personal/top_up_edit/',array('id'=>$vo['id'],'user_id'=>$vo['user_id'],'status'=>'2'));?>" href="javascript:void(0)" class="no">拒绝 </a> ]
                                    <?php else: ?>   
                                        <?php if($vo['status'] == 1): ?>已通过<?php elseif($vo['status'] == 2): ?>已拒绝<?php endif; endif; ?>
                                    [ <a href="<?php echo U('Personal/top_up_detail/',array('id'=>$vo['id'],'user_id'=>$vo['user_id']));?>" >详细信息</a> ] 
                                </td>
                            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                        <tr><td colspan="8" align="left">
                            <form action="<?php echo U('Personal/export_top');?>" method="post">
                                        日期：
