...
class SiteinfoController extends CommonController {
        ...
        if(IS_POST){
            ...
            if($_FILES['ad_img']['name']){
                ...
                if(!$info) {// 上传错误提示错误信息
                    ...
                    $data['ad_img'] = '/Uploads/picture/'.$info['savename'];
                }
            }
            $m_ad=M('ad');
            if($id){
    ...
    public function message(){
        ...
        foreach ($list as $key => $value) {
            $list[$key]['personal'] = M('personal')->where("user_id = {$value['uid']}")->field('user_id,user_tel,user_nickname')->find();
        }
        $this->assign("page", $showPage);
        $this->assign("list",$list);
        $this->display();
    ...
    public function message_info()
    {
        $M = M("idea");
        $id=I('get.id');
        $M->where("id = {$id}")->save(['status'=>1]);
        $list=$M->where("id = {$id}")->find();
        $list['personal'] = M('personal')->where("user_id = {$list['uid']}")->field('user_id,user_tel,user_nickname')->find();
    ...
    public function pay_add(){
        if(IS_POST){
            ...
            $id = M('pay')->add($post);
            if($id){
                $this->success('添加成功！',U('Siteinfo/pay_index'));
            }else{
                $this->error('添加失败！');
            }
        }else{
    ...
    public function pay_edit(){
        if(IS_POST){
            ...
            $id = M('pay')->save($post);
            if($id){
                $this->success('修改成功！',U('Siteinfo/pay_index'));
            }else{
                $this->error('修改失败！');
            }
        }else{
            $where['pay_id'] = I('get.id');
            $info = M('pay')->where($where)->find();
            $this->assign('info',$info);
            $this->display();
        }
    }
    ...
    public function agreement(){
        if(IS_POST){
          ...
          $data['id'] = 1;
          $res = M('agreement')->save($data);
          if($res){
            $this->success('修改成功',U('agreement'));
          }else{
            $this->error('修改失败');
          }
        }else{
            $info = M('agreement')->where('id = 1')->find();
