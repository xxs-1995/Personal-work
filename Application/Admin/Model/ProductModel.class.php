...
class ProductModel extends Model {
    ...
    public function listProduct($firstRow = 0, $listRows = 20,$__map) {
        $M = M("Product");
        $list = $M->field("`id`,`title`,`status`,`psize`,`published`,`cid`,`price`,`release`,`aid`,`is_recommend`,`lang`,`wap_display`")->where($__map)->order("`published` DESC")->limit("$firstRow , $listRows")->select();
